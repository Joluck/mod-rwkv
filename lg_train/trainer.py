...
class train_callback(pl.Callback):
    ...
    def write_data(self, loss_data, t_cost, kt_s, lr):
        # 将loss数据写入文件，便于streamlit绘图
        with open(self.loss_file, 'a') as f:
            json.dump({"loss": float(loss_data), "t_cost": t_cost, "kt_s": kt_s, "lr": lr}, f)
            f.write('\n')
        ...
        if trainer.is_global_zero:  # logging
            ...
            if trainer.accumulate_grad_batches!=None:
                ...
                if (batch_idx+1) % trainer.accumulate_grad_batches == 0:
                    ...
                    self.write_data(args.avg_loss, t_cost, kt_s,trainer.my_lr)
                    args.avg_loss = 0
            else:
                if len(args.wandb) > 0:
